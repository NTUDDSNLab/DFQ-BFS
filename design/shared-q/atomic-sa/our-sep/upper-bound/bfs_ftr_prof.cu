...
// shared-q/atomic-sa/our-sep/upper-bound/

#define MAX_ITER 2000

#define TILE_LOG_A      4
#define TILE_SIZ_A     16
#define TILE_RNG_A     16
#define FTR_SIZE_A   4608

#define TILE_LOG_B      7
#define TILE_SIZ_B    128
#define TILE_RNG_B    128
#define FTR_SIZE_B   3584

#define TILE_LOG_C     10
#define TILE_SIZ_C   1024
#define TILE_RNG_C   1024
#define FTR_SIZE_C   2560

#define TILE_LOG_E     13
#define TILE_SIZ_E   8192
#define TILE_RNG_E   8192
#define FTR_SIZE_E 131072
...
__device__ int idle_Q[MAX_ITER][5];
...
{
    __shared__ int ftr_AQ[FTR_SIZE_A], ftr_BQ[FTR_SIZE_B], ftr_CQ[FTR_SIZE_C];
    __shared__ int ftr_size_AQ, ftr_size_BQ, ftr_size_CQ, node_i;
    ...
    if(threadIdx.x + blockIdx.x * blockDim.x == 0){
        ...
        printf("Siz: %d %d %d %d %d\n" , TILE_SIZ_A, TILE_SIZ_B, TILE_SIZ_C, TILE_SIZ_E, gridDim.x * blockDim.x);
        printf("Rng: %d %d %d %d inf\n", TILE_RNG_A, TILE_RNG_B, TILE_RNG_C, TILE_RNG_E);
    }
    grid.sync();
    
    for(short iter = 0; !(*done) || !iter; dist = ++iter){
        if(threadIdx.x == 0){
            // if(iter == 0) idle_AQ[blockIdx.x] = idle_BQ[blockIdx.x] = idle_CQ[blockIdx.x] = 0;
            ftr_size_AQ = ftr_size_BQ = ftr_size_CQ = 0;
            node_i = blockIdx.x * blockDim.x;
        }
        if(threadIdx.x + blockIdx.x * blockDim.x == 0){
            // if(iter == 0) idle_EQ = idle_FQ = 0;
            ftr_size_EQ = ftr_size_FQ = 0;
            NODE_i = gridDim.x * blockDim.x;
        }
        if(blockIdx.x == 0 && threadIdx.x < 5){
            idle_Q[iter][threadIdx.x] = 0;
        }
        grid.sync();
        *done = true;
        while(node_i < *NUM_NODES){
            ...
            if(idx < *NUM_NODES && cost[idx] == iter && node[idx].length){
                ...
                (node[idx].length <= TILE_RNG_A) ? atomicAdd(&(idle_Q[iter][0]), TILE_SIZ_A - (node[idx].length & ~(0xffffffff << TILE_LOG_A))) : \
                (node[idx].length <= TILE_RNG_B) ? atomicAdd(&(idle_Q[iter][1]), TILE_SIZ_B - (node[idx].length & ~(0xffffffff << TILE_LOG_B))) : \
                (node[idx].length <= TILE_RNG_C) ? atomicAdd(&(idle_Q[iter][2]), TILE_SIZ_C - (node[idx].length & ~(0xffffffff << TILE_LOG_C))) : \
                (node[idx].length <= TILE_RNG_E) ? atomicAdd(&(idle_Q[iter][3]), TILE_SIZ_E - (node[idx].length & ~(0xffffffff << TILE_LOG_E))) : \
                                                   atomicAdd(&(idle_Q[iter][4]), gridDim.x * blockDim.x - (node[idx].length % (gridDim.x * blockDim.x))) ;
            }
            __syncthreads();
    ...
    grid.sync();
    if(threadIdx.x + blockIdx.x * blockDim.x == 0){
        // for(int j = 0; j < gridDim.x; j++) j ? printf(" %d", idle_AQ[j]) : printf("\n%d", idle_AQ[j]);
        // for(int j = 0; j < gridDim.x; j++) j ? printf(" %d", idle_BQ[j]) : printf("\n%d", idle_BQ[j]);
        // for(int j = 0; j < gridDim.x; j++) j ? printf(" %d", idle_CQ[j]) : printf("\n%d", idle_CQ[j]);
        // printf("\n%d\n%d", idle_EQ, idle_FQ);
        for(short j = 0; j < dist; j++){
            printf("%d: %d %d %d %d %d\n", j, idle_Q[j][0], idle_Q[j][1], idle_Q[j][2], idle_Q[j][3], idle_Q[j][4]);
        }
    }
    grid.sync();
}
