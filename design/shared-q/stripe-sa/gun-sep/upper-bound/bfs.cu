...
// shared-q/stripe-sa/gun-sep/upper-bound/

#define FTR_SIZE_S 4608
#define FTR_SIZE_M 3584
#define FTR_SIZE_L 8388608

#define TILE_LOG_S   5
#define TILE_SIZ_S  32
#define TILE_RNG_S  32

#define TILE_LOG_M   8
#define TILE_SIZ_M 256
...
__global__ void CUDA_BFS_KERNEL_CG_NEW(Node *node, int *edge, short *cost, bool *done, int *NUM_NODES)
{
    ...
    for(short iter = 0; !(*done) || !iter; iter++){
        ...
        grid.sync();
        *done = true;
        for(int i = blockIdx.x * blockDim.x; i < *NUM_NODES; i += blockDim.x * gridDim.x){
            ...
            __syncthreads();
            if(ftr_size_SQ >= FTR_SIZE_S - blockDim.x){
                for(int j = threadIdx.x >> TILE_LOG_S; j < ftr_size_SQ; j += blockDim.x >> TILE_LOG_S){
            ...
            if(ftr_size_MQ >= FTR_SIZE_M - blockDim.x){
                for(int j = threadIdx.x >> TILE_LOG_M; j < ftr_size_MQ; j += blockDim.x >> TILE_LOG_M){
        ...
        grid.sync();
        for(int j = 0; j < ftr_size_LQ; j++){
            VISIT_all_tiled(node, edge, cost, done, ftr_LQ[j]);
        }
        grid.sync();
    }
}
