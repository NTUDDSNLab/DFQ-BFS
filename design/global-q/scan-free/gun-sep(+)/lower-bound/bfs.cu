...
__device__ inline void VISIT_tiled(thread_group tile, Node *node, int *edge, unsigned int *cost, bool *done, int ftr, int *ftr_SQ, int *ftr_size_SQ, int *ftr_MQ, int *ftr_size_MQ, int *ftr_LQ, int *ftr_size_LQ)
{
    ...
    for(int i = start+tile.thread_rank(); i < end; i += tile.size()){
        ...
        if(atomicCAS(&(cost[nid]), (unsigned int)(0xffffffff), (unsigned int)(cost[ftr] + 1)) == (unsigned int)(0xffffffff)){
            if(node[nid].length) {
                ...
                (node[nid].length <= TILE_RNG_S) ? ftr_SQ[atomicAdd(ftr_size_SQ, 1)] = nid : \
                (node[nid].length <= TILE_RNG_M) ? ftr_MQ[atomicAdd(ftr_size_MQ, 1)] = nid : \
                                                   ftr_LQ[atomicAdd(ftr_size_LQ, 1)] = nid ;
...
__device__ inline void VISIT_non_tiled(short log_size, Node *node, int *edge, unsigned int *cost, bool *done, int ftr, int *ftr_SQ, int *ftr_size_SQ, int *ftr_MQ, int *ftr_size_MQ, int *ftr_LQ, int *ftr_size_LQ)
{
    int start = node[ftr].start;
    int end = start + node[ftr].length;
...
__device__ int ftr_MQ_a[FTR_SIZE_M], ftr_MQ_b[FTR_SIZE_M];
__device__ int ftr_size_MQ_a, ftr_size_MQ_b;
__device__ int *ftr_MQ_f, *ftr_MQ_t;
__device__ int ftr_LQ_a[FTR_SIZE_L], ftr_LQ_b[FTR_SIZE_L];
...
__global__ void CUDA_BFS_KERNEL_CG_NEW(Node *node, int *edge, unsigned int *cost, bool *done, int *NUM_NODES)
{
    ...
    for(short iter = 0; !(*done) || !iter; iter++){
        if(threadIdx.x + blockIdx.x * blockDim.x == 0){
            ...
            (iter % 2) ? (ftr_size_SQ_b = 0) : (ftr_size_SQ_a = 0);
            (iter % 2) ? (ftr_size_MQ_b = 0) : (ftr_size_MQ_a = 0);
            (iter % 2) ? (ftr_size_LQ_b = 0) : (ftr_size_LQ_a = 0);
            // printf("size a/b: %d %d\n", ftr_size_SQ_a, ftr_size_SQ_b);
            ftr_SQ_f = ((iter % 2) ? ftr_SQ_a : ftr_SQ_b);
            ftr_SQ_t = ((iter % 2) ? ftr_SQ_b : ftr_SQ_a);
            ftr_MQ_f = ((iter % 2) ? ftr_MQ_a : ftr_MQ_b);
            ftr_MQ_t = ((iter % 2) ? ftr_MQ_b : ftr_MQ_a);
            ftr_LQ_f = ((iter % 2) ? ftr_LQ_a : ftr_LQ_b);
            ftr_LQ_t = ((iter % 2) ? ftr_LQ_b : ftr_LQ_a);
        }
        grid.sync();
        *done = true;
        grid.sync();
        for(int j = ((threadIdx.x + blockIdx.x * blockDim.x) >> TILE_LOG_S);
        j < ((iter % 2) ? ftr_size_SQ_a : ftr_size_SQ_b);
        j += (blockDim.x * gridDim.x) >> TILE_LOG_S){
            VISIT_tiled(tile_S, node, edge, cost, done, ftr_SQ_f[j],
            ftr_SQ_t, ((iter % 2) ? &ftr_size_SQ_b : &ftr_size_SQ_a),
            ftr_MQ_t, ((iter % 2) ? &ftr_size_MQ_b : &ftr_size_MQ_a),
            ftr_LQ_t, ((iter % 2) ? &ftr_size_LQ_b : &ftr_size_LQ_a));
        }

        for(int j = ((threadIdx.x + blockIdx.x * blockDim.x) >> TILE_LOG_M);
        j < ((iter % 2) ? ftr_size_MQ_a : ftr_size_MQ_b);
        j += (blockDim.x * gridDim.x) >> TILE_LOG_M){
            VISIT_non_tiled(TILE_LOG_M, node, edge, cost, done, ftr_MQ_f[j],
            ftr_SQ_t, ((iter % 2) ? &ftr_size_SQ_b : &ftr_size_SQ_a),
            ftr_MQ_t, ((iter % 2) ? &ftr_size_MQ_b : &ftr_size_MQ_a),
            ftr_LQ_t, ((iter % 2) ? &ftr_size_LQ_b : &ftr_size_LQ_a));
        }

        for(int j = ((threadIdx.x + blockIdx.x * blockDim.x) >> TILE_LOG_L);
        j < ((iter % 2) ? ftr_size_LQ_a : ftr_size_LQ_b);
        j += (blockDim.x * gridDim.x) >> TILE_LOG_L){
            VISIT_non_tiled(TILE_LOG_L, node, edge, cost, done, ftr_LQ_f[j],
            ftr_SQ_t, ((iter % 2) ? &ftr_size_SQ_b : &ftr_size_SQ_a),
            ftr_MQ_t, ((iter % 2) ? &ftr_size_MQ_b : &ftr_size_MQ_a),
            ftr_LQ_t, ((iter % 2) ? &ftr_size_LQ_b : &ftr_size_LQ_a));
        }
