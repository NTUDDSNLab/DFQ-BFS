...
// global-q/atomic-sa/non-sep/ombre-bound/

#define FTR_SIZE_M 33554432

#define TILE_LOG_M  6
#define TILE_SIZ_M 64
...
__device__ int ftr_MQ[FTR_SIZE_M];
__device__ int ftr_size_MQ, NODE_i;
        ...
        *done = true;
        ...
        while(node_i < *NUM_NODES){
            ...
            if(idx < *NUM_NODES && cost[idx] == iter && node[idx].length){
                ftr_MQ[atomicAdd(&ftr_size_MQ, 1)] = idx;
            }
            if(threadIdx.x == 0){
